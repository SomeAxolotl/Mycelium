...
public class BlitzSkill : MonoBehaviour
{
    // cooldown 8 seconds 
    // unlock at 5
    // damage 20
    ...
    float blitzDmg = 25f;
    public float finalBlitzDmg;
    float blitzCooldown = 5f;
    ...
    void Update()
    {
        ...
        if (skillLoadout.transform.GetChild(1).name == "BlitzSkill")
        {
    ...
    IEnumerator FetchStats()
    {
        ...
        finalBlitzDmg = sentienceSkillDmg + blitzDmg;

        //skillCooldownBuff = player.GetComponent<CharacterStats>().skillCooldownBuff;
        finalSkillCooldown = blitzCooldown - skillCooldownBuff;
