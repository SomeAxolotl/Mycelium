...
public class BossMeleeAttack : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        foreach (var playerCollider in playerColliders)
        {
            ...
            if (Vector3.Angle(transform.forward, dirToPlayer) < 30f && !Physics.Raycast(transform.position, dirToPlayer, dstToPlayer, obstacleLayer) && (dstToPlayer - 0.5f) <= navMeshAgent.stoppingDistance && canAttack && !windupStarted)
            {
